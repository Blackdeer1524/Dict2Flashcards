...
def define(content, dictionary_index=0):
    """
    ...
    if len(primal_block) >= dictionary_index + 1:
        non_idiom_blocks = primal_block[dictionary_index].find_all("div", {"class": "pr entry-body__el"})
        non_idiom_blocks.extend(primal_block[dictionary_index].find_all("div", {"class": "pv-block"}))
        idiom_blocks = primal_block[dictionary_index].find_all("div", {"class": "pr idiom-block"})
    else:
        return {}

    for is_idiom_block, main_blocks in enumerate((non_idiom_blocks, idiom_blocks)):
        for entity in main_blocks:
            ...
            if is_idiom_block:
                pos = "idiom"
            elif pos_block is None:
                pos = ""
            else:
                pos = pos_block.text.strip()
...
if __name__ == "__main__":
    ...
    with open(r"C:\Users\Danila\Desktop\sentence_mining\dictionaries\cambridge\cambridge_htmls\k\keep-your-eyes-peeled-skinned_164254489108.html", "r", encoding="Utf-8") as f:
        content = f.read()
    pprint(define(content))
