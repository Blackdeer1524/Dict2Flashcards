...
def translate_word(word_dict):
    ...
    for word in word_dict:
        for pos in word_dict[word]:
            ...
            audio = word_dict[word][pos].get("US_audio_link", "")
            for definition, examples, domain, labels_and_codes, level, \
                region, usage, image, alt_terms in zip(word_dict[word][pos]["definitions"],
                                                       word_dict[word][pos]["examples"],
                                                       word_dict[word][pos]["domain"],
                                                       word_dict[word][pos]["labels_and_codes"],
                                                       word_dict[word][pos]["level"],
                                                       word_dict[word][pos]["region"],
                                                       word_dict[word][pos]["usage"],
                                                       word_dict[word][pos]["image_links"],
                                                       word_dict[word][pos]["alt_terms"]):
...
def get_idiom(soup):
    ...
    if not phrasal_main_block:
        ...
        if idiom_main_block is not None:
            ...
            for def_and_sent_block in idiom_main_block.find_all("div", {"class": "def-block ddef_block"}):
