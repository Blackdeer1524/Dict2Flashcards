...
class App(Tk):
    ...
    def start_image_search(self):
        def connect_images_to_card(instance):
            ...
            self.JSON_CONF_FILE["app"]["image_search_position"] = f"+{image_search_pos[0]}+{image_search_pos[1]}"
    ...
    def save_files(self):
        ...
        root_pos = self.winfo_geometry().split(sep="+")[1:]
        self.JSON_CONF_FILE["app"]["main_window_position"] = f"+{root_pos[0]}+{root_pos[1]}"
    ...
    def change_media_dir(self, raise_error=True):
        new_media_dir = askdirectory(title="Выберете директорию collection.media в USERNAME/",
                                     initialdir=os.getenv('APPDATA') + "/Anki2")
